...
export const playNext = async (playbackObj, url) => {
  print(url)
  try {
    await playbackObj.stopAsync();
    await playbackObj.unloadAsync();
    return await play(playbackObj, url);
  } catch (error) {
...
export const selectAudio = async (audio, context, playListInfo = {}) => {
  ...
  try {
    ...
    if (soundObj === null) {
      const status = await play(playbackObj, env.host + audio.url, audio.lastPosition);
    ...
    if (soundObj.isLoaded && currentAudio.id !== audio.id) {
      const status = await playNext(playbackObj, env.host + audio.url);
      const index = audioFiles.findIndex(({ id }) => id === audio.id);
      updateState(context, {
        currentAudio: audio,
        soundObj: status,
  ...
  try {
    ...
    if (select === 'next') {
      ...
      if (!isLoaded && !isLastAudio) {
        ...
        status = await play(playbackObj, env.host + audio.url);
        playbackObj.setOnPlaybackStatusUpdate(onPlaybackStatusUpdate);
      }

      if (isLoaded && !isLastAudio) {
        index = currentAudioIndex + 1;
        status = await playNext(playbackObj, env.host + audio.url);
      }
